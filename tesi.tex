...
\begin{tabular}{| l | c | c |}
    \hline
    \textbf{Test} & \textbf{Automatico} & \textbf{Manuale}\footnotemark \\ \hline
    \textbf{Tempo di setup}\footnotemark & 0m 5s & circa 10 minuti \\\hline
...
\footnotetext[1]{Include oltre al tempo per svolgere il test il tempo necessario per leggere i passi da svolgere,
    assicurarsi che il RE sia alle condizioni iniziali, ed alla fine del test aggiornare il documento con l'esito}
\footnotetext[2]{Tempo necessario per predisporre l'ambiente ed il materiale per poter iniziare a svolgere i test}
\footnotetext[3]{Tempo di esecuzione lanciando assieme tutti i test, 
    potrebbe non corrispondere esattamente alla somma dei tempi di esecuzione dei singoli test}

Come vediamo si passa da un tempo di esecuzione manuale di circa mezz'ora ad un tempo di esecuzione automatica 
di poco meno di 5 minuti. Ovviamente il vero risparmio non è il tempo in sè, ma il fatto che non vi è più la necessità 
di un operatore che supervisioni le operazioni, e che possa commettere errori nello svolgerle. 

Ho voluto testare l'infrastruttura con varie versioni del software, inclusa una che non è ancora stata rilasciata. 
Grazie ai test automatizzati, e questo non è stato fatto apposta, mi sono potuto rendere conto di un bug nella gestione 
dei LED di illuminazione della pulsantiera che facevano sì che in certe casistiche non venissero spente. 
